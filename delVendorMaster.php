	...
	//{
		...
		if(mysql_affected_rows()==1)
		{
			...
			*/
			...
			$sqlSelect = mysql_query("select * from ba_tbl_vendor_master where id = '$id'");
			...
			$sqlContentSelect = mysql_query("select * from ba_tbl_content where vendor_id = '$updated_id'"); //Selecting all updated ids related to vendor master
	
			while($rowContentSelect = mysql_fetch_assoc($sqlContentSelect))
			{
				//$user_id = $rowContentSelect[""]
				...
				//echo "USER SPACE : "  .$user_space_used."<br>";
				$content_size = $rowContentSelect["content_size"];	
				//echo "CONTENT SIZE : " . $content_size . "<br>";
				$new_space = $user_space_used - $content_size;
				//echo "NEW SPACE : " . $new_space. "<br>";

				/** Updating user table with user user_space_used **/
				//$last_modified = date("Y-d-m h:i:s");
				$sqlUpdate = mysql_query("update ba_tbl_user set user_space_used = '$new_space' where id = '$user_id'");
				/*** END ***/	
