...
if(isset($_REQUEST['user_id']) && $_REQUEST['user_id']!="")
{
	...
	if($gs_size>0)
	{
		...
		$sqlUpdate = mysql_query("update ba_tbl_user set f_name = '$f_name', l_name = '$l_name', gender = '$gender', contact_no = '$contact_no', dob = '$dob', last_modified = '$last_modified', profile_image = '$profile_pic_url' where id = '$user_id'") or die(mysql_error());
		if(mysql_affected_rows()==1)
		{
			echo '[{"response":"updated", "user_id":"'.$user_id.'", "profile_pic_url" : "'.$profile_pic_url.'", "last_modified" : "'.$last_modified.'"}]';
		}
		else
		{
	...
	{
		$last_modified = date("Y-d-m h:i:s");
		$sqlUpdate = mysql_query("update ba_tbl_user set f_name = '$f_name', l_name = '$l_name', gender = '$gender', contact_no = '$contact_no', dob = '$dob', last_modified = '$last_modified' where id = '$user_id'") or die(mysql_error());
		if(mysql_affected_rows()==1)
		{
			echo '[{"response":"updated", "user_id":"'.$user_id.'", "last_modified" : "'.$last_modified.'"}]';
		}
		else
		{
			echo '[{"response":"no update", "user_id":"'.$user_id.'"}]';
